#!/bin/bash
VALID_ARGS=$(getopt -o f:npux:l:ch --long clean,format:,no-internet,package,un-package,prefix:,line-number:,help -- "$@")
if [[ $? -ne 0 ]]; then
    exit 1;
fi
    ...
    do
        ...
        then
            ...
            then
                ...
                echo "Try to fix error and start this script again with '-l $line_number' option. Exiting..."
                echo "$line_number" > $myDir/deploy.config
                echo "$prefix" >> $myDir/deploy.config
                exit 1
            fi
